...
public final class VoteTopGui extends AbstractGui {

    private static final int ROWS = 6;
    private static final int CLOSE_SLOT = 49;

    public VoteTopGui(OneSMPPlugin plugin, List<VoteManager.TopEntry> entries) {
        super(plugin, plugin.messages().text("vote.gui-title"), ROWS);

        for (int i = 0; i < entries.size() && i < 45; i++) {
            VoteManager.TopEntry entry = entries.get(i);
            ItemStack head = new ItemBuilder(Material.PLAYER_HEAD)
                    .name(plugin.messages().text("vote.gui-rank-name",
                    ...
                    .lore(plugin.messages().text("vote.gui-value-lore",
                            Placeholder.unparsed("value", String.valueOf(entry.value()))))
                    .build();
